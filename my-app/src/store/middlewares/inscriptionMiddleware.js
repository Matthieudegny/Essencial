...
import {requestInscriptionForm, requestInscriptionFormVillage /* saveAuthorization, */ } from '../../requests/';
 ...
 switch (action.type) {
   case SUBMIT_FORM: {
    ...
    console.log('logstate.inscription',state.inscription);
     const { last_name_manager, first_name_manager, date_of_birth_manager, description, website, phone_number, name, last_name, first_name, email, pseudo, password, address, region, zip_code, city, path } = state.inscription;
     
     
    
     console.log('je fait mon getState pour recuperer', { last_name_manager, first_name_manager, date_of_birth_manager, description, website, phone_number, name,last_name, first_name, email, pseudo, password, address, region, zip_code, city, path});
     ...
     try {
       // on execute la requete POST /createUser
         ...
         const { ecovillage, user, photo} = await requestInscriptionForm( last_name, first_name, email, pseudo, password, address, region, zip_code, city, path );
         console.log("la requete est terminé et j'ai récupéré:", { ecovillage,user,photo});
       ...
       try {
        // on execute la requete POST /createVillage
          ...
          const { ecovillage, photo} = await requestInscriptionFormVillage( last_name_manager, first_name_manager, date_of_birth_manager, description, website, phone_number, name, email, password, address, region, zip_code, city, path );
          ...
          console.log("je dispatch SAVE_USER avec les infos de l'utilisateur connecté");
          store.dispatch(
            actionSaveUser(last_name_manager, first_name_manager, date_of_birth_manager, description, website, phone_number, name, last_name, first_name, email, pseudo, password, address, region, zip_code, city, path),
          );
        }
        catch (err) {
        // on capture les eventuelles erreur de la requete
