 ...
 switch (action.type) {
   case SUBMIT_POST: {
    ...
    console.log('logstate.addpost',state.addpost);
     const { title, content, path } = state.addpost;
     
     
    
     console.log('je fait mon getState pour recuperer', { title, content, path });
     ...
     try {
       // on execute la requete POST /createAddPost
         ...
         const { post, photo } = await requestAddPost(title, content, path );
         ...
         store.dispatch(
           actionSavePost(title, content, path),
         );
       }
       catch (err) {
